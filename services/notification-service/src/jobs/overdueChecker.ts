...
export class OverdueChecker {
  ...
  private checkIntervalMs: number = 15 * 60 * 1000;
  ...
  async runCheck(): Promise<void> {
    try {
      ...
      const overdueResult = await this.db.query(`
        SELECT cp.id as capture_point_id, cp.name as capture_point_name, cp.capture_frequency_hours,
               cp.zone_id, z.name as zone_name, z.project_id, p.name as project_name, p.tenant_id,
               p.alert_delay_threshold_days, p.alert_critical_delay_threshold_days,
               MAX(ph.captured_at) as last_capture_at, COUNT(ph.id) as total_photos
        ...
        WHERE cp.is_active = true AND p.status = 'active'
        GROUP BY cp.id, cp.name, cp.capture_frequency_hours, cp.zone_id, z.name, z.project_id,
                 p.name, p.tenant_id, p.alert_delay_threshold_days, p.alert_critical_delay_threshold_days
        HAVING MAX(ph.captured_at) IS NULL OR (NOW() - MAX(ph.captured_at)) > (cp.capture_frequency_hours || ' hours')::interval
      `);
  ...
  private async processOverduePoint(point: any): Promise<void> {
    ...
    const usersResult = await this.db.query(`
      ...
      WHERE u.tenant_id = $1 AND u.role IN ('project_manager', 'tenant_admin', 'super_admin') AND u.is_active = true
    `, [point.tenant_id]);
    ...
    for (const user of users) {
      try {
        await this.sendNotification({
          ...
          type: isCritical ? 'critical_delay' : 'delay_alert', severity: isCritical ? 'critical' : 'warning',
          projectId: point.project_id, projectName: point.project_name,
          capturePointId: point.capture_point_id, capturePointName: point.capture_point_name,
          zoneName: point.zone_name, daysBehind, lastCaptureAt: point.last_capture_at,
          channels: isCritical ? ['push', 'email', 'in_app'] : ['push', 'in_app'],
        });
      } catch (err) {
        logger.error({ message: 'Failed to send notification', userId: user.id, capturePointId: point.capture_point_id, error: (err as Error).message });
      }
    }
    logger.info({ message: 'Overdue point notifications sent', capturePointId: point.capture_point_id, capturePointName: point.capture_point_name, daysBehind, isCritical, recipients: users.length });
  }

  private async sendNotification(payload: any): Promise<void> {
    const title = payload.severity === 'critical' ? `⚠️ تأخير حرج: ${payload.capturePointName}` : `🔔 نقطة متأخرة: ${payload.capturePointName}`;
    ...
    const htmlBody = `<div dir="rtl" style="font-family: Cairo, Arial, sans-serif; text-align: right;"><h2>${title}</h2><p><strong>المشروع:</strong> ${payload.projectName}</p><p><strong>المنطقة:</strong> ${payload.zoneName}</p><p><strong>نقطة التقاط:</strong> ${payload.capturePointName}</p><p><strong>الأيام المتأخرة:</strong> ${payload.daysBehind}</p><p><strong>آخر صورة:</strong> ${payload.lastCaptureAt ? new Date(payload.lastCaptureAt).toLocaleDateString('ar-SA') : 'لم يتم التقاطها بعد'}</p><hr><p style="font-size: 12px; color: #666;">هذا الإشعار تم إنشاؤه تلقائياً من قبل منصة شاهد.<br>SRS v1.0 | Year 1 MVP</p></div>`;

    await axios.post(`${this.notificationApiUrl}/send`, {
      user_id: payload.userId, type: payload.type, channels: payload.channels,
      payload: { title, body, html: htmlBody, severity: payload.severity, project_id: payload.projectId, capture_point_id: payload.capturePointId, days_behind: payload.daysBehind, last_capture_at: payload.lastCaptureAt, zone_name: payload.zoneName },
    }, { timeout: 10000 });
  }
}
