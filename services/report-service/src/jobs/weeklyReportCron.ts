...
export class WeeklyReportCron {
  ...
  start(): void {
    ...
    logger.info({ message: 'Weekly report cron scheduled', nextRun: nextRun.toISOString(), delayMs: delay });
  ...
  private getNextMonday0800(): Date {
    ...
    if (day === 1 && now.getHours() >= 8) { nextMonday.setDate(nextMonday.getDate() + 7); }
    return nextMonday;
  }
  ...
  async runJob(): Promise<void> {
    try {
      ...
      const projectsResult = await this.db.query(
        `SELECT id, report_language, capture_frequency_hours FROM projects WHERE status = 'active'`
      );
      ...
      for (const project of projects) {
        ...
        const jobData = {
          jobId, project_id: project.id,
          period_start: periodStart.toISOString().split('T')[0],
          period_end: periodEnd.toISOString().split('T')[0],
          triggered_by: 'auto', queued_at: new Date().toISOString(),
        };

        await this.redis.lpush('report:queue', JSON.stringify(jobData));
        logger.info({ message: 'Weekly report queued', jobId, projectId: project.id, periodStart: jobData.period_start, periodEnd: jobData.period_end });
      }

      logger.info({ message: 'Weekly report cron job completed', projectsQueued: projects.length });
    } catch (err) {
  ...
  async triggerManual(projectId: string, triggeredBy: string): Promise<string> {
    const now = new Date();
    const periodEnd = new Date(now); periodEnd.setHours(8, 0, 0, 0);
    const periodStart = new Date(periodEnd); periodStart.setDate(periodEnd.getDate() - 7);
    const jobId = `report:${projectId}:${Date.now()}:manual`;
    const jobData = { jobId, project_id: projectId, period_start: periodStart.toISOString().split('T')[0], period_end: periodEnd.toISOString().split('T')[0], triggered_by: triggeredBy, queued_at: new Date().toISOString() };
    await this.redis.lpush('report:queue', JSON.stringify(jobData));
    return jobId;
  }
}
